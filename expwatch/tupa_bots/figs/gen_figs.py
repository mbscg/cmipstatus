...
def run_script(params, cmp):
    ...
    if cmp:
        command = command.format(ferret, script_v2, orig_nc, variable, final_year, cmp['output_nc'], plot_area)
    else:
        command = command.format(ferret, script, orig_nc, variable, final_year)
